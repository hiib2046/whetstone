...
### 브랜치
`main` 하나로 간다. 작업은 바로 `main`에 커밋하고, 릴리스는 태그로 표시한다(아래 릴리스 절차). 솔로 관리라 브랜치 격리 비용이 이득보다 크다 — 실험이 커지면 그때 브랜치를 판다.
...
**scope**(선택) — 어디를 건드렸나. 최상위 디렉토리명을 쓴다: `guides`·`skills`·`fixtures`·`docs`·`plugin`(메타 파일). 레포 전체에 걸치면 생략.
...
**타입과 버전의 연결:** `feat` → 보통 MINOR · `fix` → 보통 PATCH · `BREAKING CHANGE`(또는 `feat!`처럼 `type!`) → MAJOR.
...
```
fix(skills): harness-inspect 묶기의 이름 유사 오탐 수정

같은 폴더·같은 런타임이라는 이유만으로 '관련 추정'에 올라
사용자가 매번 진짜 관련인지 되확인해야 했다. 이름·주제
유사성이 있을 때로만 좁혔다.
```
