 ...
 * 입출력: 인자 2개 — [1] 입력 JSON 경로, [2] 출력 HTML 경로. stdin 미사용.
 *   성공 시 생성된 HTML 경로를 stdout에 한 줄 찍고 exit 0(호출자가 산출물 생성을 별도 확인 없이 알 수 있게).
 *   인자 누락·파일 읽기 실패·JSON 파싱 실패 시 stderr에 메시지 후 exit 1.
...
function main() {
  ...
  // 성공을 침묵으로 끝내면 호출자가 산출물이 생겼는지 별도 확인해야 한다 — 경로를 찍어 그 확인을 없앤다.
